...
// 책의 이름을 이용한 책 찾기
bool Book::searchBook(vector<Book>& bookList, vector<Book>& searchBookList, string title){
    bool flag = false;
    for(int i=0;i<bookList.size();i++){
        if(bookList[i].getTitle() == title){
            this->setBookInfo(bookList[i].getTitle(),bookList[i].getAuthor(),bookList[i].getPublisher());
            searchBookList.push_back(bookList[i]);
            flag=true;
        }
    }
    if(flag)
...
// 동일한 isbn이 존재하는지 확인
bool Book::checkSameISBN(vector<Book>& bookList,int isbn){
    for(int i=0;i<bookList.size();i++){
        for(int j=0;j<bookList[i].each.size();j++){
            if(isbn==bookList[i].each[j].getISBN())
                return false;
        }
    }
    return true;
...
bool checkTitle(vector<Book>& bookList, vector<Book>& includeList, string str){
    bool flag = false;
    for(int i=0;i<bookList.size();i++){
        if(bookList[i].getTitle().find(str)!= string::npos){
            includeList.push_back((bookList[i]));
            flag=true;
        }
    }
